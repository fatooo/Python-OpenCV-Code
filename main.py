...
while(True):
    ...
    data.append(frame)
    #out.write(frame)
    #cv2.imshow('crop', template_w), cv2.waitKey(0), cv2.destroyAllWindows()
    template_w,velocity_x_w,velocity_y_w,velocity_w,data,data_w,x_start_w,y_start_w,x_stop_w,y_stop_w = white_track(frame,i,data,data_w,template_w,x_prime_w,y_prime_w,velocity_x_w,velocity_y_w,velocity_w,x_start_w,y_start_w,x_stop_w,y_stop_w)
    template_y,velocity_x_y,velocity_y_y,velocity_y,data,data_y,x_start_y,y_start_y,x_stop_y,y_stop_y = yellow_track(frame,i,data,data_y,template_y,x_prime_y,y_prime_y,velocity_x_y,velocity_y_y,velocity_y,x_start_y,y_start_y,x_stop_y,y_stop_y)
    template_r,velocity_x_r,velocity_y_r,velocity_r,data,data_r,x_start_r,y_start_r,x_stop_r,y_stop_r = red_track(frame,i,data,data_r,template_r,x_prime_r,y_prime_r,velocity_x_r,velocity_y_r,velocity_r,x_start_r,y_start_r,x_stop_r,y_stop_r)
    demo(frame,i,data,data_w,data_y,data_r)
    i = i + 1
