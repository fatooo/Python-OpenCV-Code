def white_track(side_touch,frame,i,data,data_w,template_o,template_2_o,x_prime,y_prime,velocity_x,velocity_y,velocity,x_start,y_start,x_stop,y_stop):
    ...
    if (i>-1):
        ...
        if i == 0:
            ...
            img_crop = frame[round(x_start):round(x_stop), round(y_start):round(y_stop)]
            img_crop_prev = data[i-1][round(x_start):round(x_stop), round(y_start):round(y_stop)]
            ...
            #imS = cv2.resize(data[i], (960, 540))
                ...
                #cv2.imshow('crop', img_crop_prev), cv2.waitKey(0), cv2.destroyAllWindows()
        img_crop_o = img_crop
    ...
    return template_o,velocity_x,velocity_y,velocity,data,data_w,x_start,y_start,x_stop,y_stop,center_prev,center
