...
frame_amount = 152  #152 for 20fps & 39 for 5 fps & 43 for 5fps new
...
for i in range(frame_amount):
    ...
    if i == 0:
        ...
        template = data[i][int(x_prime[i]):int(x_prime[i])+30, int(y_prime[i]):int(y_prime[i])+30] #update the template with the ball found in the current frame
...
for i in range(frame_amount):
    ...
    cv2.circle(data[frame_amount-1], (int(y_prime[i-1])+15, int(x_prime[i-1])+15), 2, (0, 0, 255), -1)
