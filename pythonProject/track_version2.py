...
w = initial_window
frame_amount = 540
template_init = cv2.imread('C:\\Users\\fatma\\PycharmProjects\\pythonProject\\Templates\\deneme_white.png')
template_2_o = cv2.imread('C:\\Users\\fatma\\PycharmProjects\\pythonProject\\Templates\\7_white.png')

template = np.float32(template_init)
template_2= np.float32(template_2_o)

template_x = template_init.shape[0]
template_y = template_init.shape[1]
...
cap = cv2.VideoCapture('C:\\Users\\fatma\\PycharmProjects\\pythonProject\\13.11.2020 videolar\\outpute7_hızlı_crop.mp4')
...
while (i<frame_amount):
        ...
        #if i>0 and velocity[i-1] < 2.75:
            #template = template_2
        #cv2.imshow('crop', template_o), cv2.waitKey(0), cv2.destroyAllWindows()
        template_2 = np.float32(template_2_o)
        res = cv2.matchTemplate(img_crop, template_2, method)
        ...
        template_x_2 = template_2_o.shape[0]
        template_y_2 = template_2_o.shape[1]
        #template_2_o = frame[int(x_prime[i]):int(x_prime[i])+template_x_2, int(y_prime[i]):int(y_prime[i]+template_y_2)]
        ...
        #cv2.imshow('final', template_2_o), cv2.waitKey(0), cv2.destroyAllWindows()
        ...
        #x_prime[i] = round(x_prime[i] + template_x_2 / 2)  # used for double template matching
        #y_prime[i] = round(y_prime[i] + template_y_2 / 2)
        #print("X prime ",x_prime[i],"Y prime ",y_prime[i])
        #print(min_loc_2[i][1], min_loc_2[i][0])
        center_prev = (
                round(y_prime[i - 1] + round(template_y_2 / 2)),
                round(x_prime[i - 1] + round(template_x_2 / 2)))

        center = (round(y_prime[i] + round(template_y_2 / 2)),
                  round(x_prime[i] + round(template_x_2 / 2)))
        print("Center ",i,center)
        print("Center previous ",i,center_prev)
        velocity_x[i] = ((center[1] - center_prev[1]) * fps / ppm_x)
        velocity_y[i] = ((center[0] - center_prev[0]) * fps / ppm_y)
        velocity[i] = (math.sqrt(math.pow(abs(velocity_x[i]), 2) + math.pow(abs(velocity_y[i]), 2)))

        cv2.circle(frame, (int(center[0]), int(center[1])), 1, (255, 0, 0), 10)
        velocity_x[0] = 0
        velocity_y[0] = 0
        velocity[0] = 0
    ...
    #data_w.append(center)
    #return template_o,velocity_x,velocity_y,velocity,data,data_w,x_start,y_start,x_stop,y_stop
