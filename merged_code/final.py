...
class Billiards:
	...
	def drawBorders(self,img_crop):
		...
		for i in range(self.w):
			if i%self.cpi == 0:
				cv2.circle(img_crop, (int(self.x+self.pew + i),self.y-self.pew), self.PointSize, self.PointCol, -1)

		for i in range(self.h):
		    if i%self.cpi == 0:
		        cv2.circle(img_crop, (self.x-self.pew,int(self.y+self.pew + i)), self.PointSize, self.PointCol, -1)
